...
target_channel_id = 1141439254329831525
kap = 1 
kapitan1 = '' 
kapitan2 = ''
voice_channel = ''
...
async def on_ready():
    ...
    voice_channel_id = 1142196358263091220  # ID вашего голосового канала
...
async def on_voice_state_update(member, before, after):
    ...
    voice_channel = client.get_channel(1141440575007101059)
    if after.channel and after.channel.id == 1141440575007101059 and after.channel != before.channel:
...
async def list_members(voice_channel):
    ...
    target_channel_id = 1141439254329831525# Замените это на ID нужного вам канала 
...
async def on_reaction_add(reaction, user):
    ...
    voice_channel = client.get_channel(1141439254329831525)
    ...
    if user.id == kapitan2_id:
        if kap == 2:
            if str(reaction) in emoji:
                ...
                target_channel = client.get_channel(1141439254329831525)  # Исправление: добавляем получение канала
                await target_channel.send(f'Игрок {selected_player} уже выбран в команду 1')
        elif kap == 1:
            await target_channel.send(f'Сейчас выбирает другой капитан')

    if len(list_com1) == 4 and len(list_com2) == 4:
        kapitan1_voice_channel_id = 1141440937852162058
        kapitan2_voice_channel_id = 1141440962661458031
        ...
        if kapitan2_member:
            await kapitan2_member.move_to(destination_channel)

        kapitan1_member = discord.utils.get(voice_channel.members, name=kap_name)
        if kapitan1_member:
            await kapitan1_member.move_to(destination_channel2)

            

client.run('MTE0MTY5OTUxNzkwNTkxMTgyOA.GeW16S.BbsW0XyuvSNGvy5_TCxfNWzuou_lH6RABobGxI')
